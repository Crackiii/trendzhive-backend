...
import { failedTasksQueue, storiesQueue, idsQueue, linksQueue, queriesQueue } from "./bull/queues"
import { FAILED_TASKS_DATA_QUEUE, IDS_DATA_QUEUE, LINKS_DATA_QUEUE, QUERY_DATA_QUEUE, STORY_DATA_QUEUE } from "./constants"
...
export const fillQueueWithData = async (queue: string, data: any) => {
  switch (queue) {
    ...
    case FAILED_TASKS_DATA_QUEUE: {
      return failedTasksQueue.add(data)
    }
  }
}
...
export const queueHasWaitingJobs = async (queue: string) => {
  switch (queue) {
    ...
    case FAILED_TASKS_DATA_QUEUE: {
      return Boolean((await failedTasksQueue.getJobCounts()).waiting)
    }
  }
}
...
export const emptyQueue = async (queue: string) => {
  switch (queue) {
    ...
    case FAILED_TASKS_DATA_QUEUE: {
      return await failedTasksQueue.clean(0)
    }
  }
}
